<x-layouts.app title="Inscription - Plombier SOS">
    <div class="max-w-md mx-auto px-4 py-12">
        ...
        <form method="POST" action="{{ route('register') }}" class="bg-white rounded-lg shadow-sm border p-6">
            @csrf
            <div class="mb-4"><label class="block text-sm font-medium mb-1">Pseudo</label><input type="text" name="pseudo" required class="w-full border rounded-lg px-3 py-2" value="{{ old('pseudo') }}"></div>
