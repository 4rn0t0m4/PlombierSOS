<x-layouts.app :title="'Plombier à ' . $city->name . ' (' . $city->postal_code . ') - Plombier SOS'">
    <div class="max-w-7xl mx-auto px-4 py-8">
        <nav class="text-sm text-gray-500 mb-6">
            ...
            <a href="{{ route('departement.show', $city->departmentRelation->slug ?? '') }}" class="hover:text-blue-600">{{ $city->departmentRelation->name ?? '' }}</a>
            <span class="mx-1">/</span>
        ...
        @if($plombiers->isNotEmpty())
            <div class="space-y-4">
                @foreach($plombiers as $plombier)
                    <x-plombier-card :plombier="$plombier" :rank="$plombier->city_ranking ?: null" />
                @endforeach
            </div>
